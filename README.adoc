...
** `uri`: Required. The URI format for linking to issues as formatted using {string_formats_link}. Default: GitHub. The `id` is dynamically calculated via the commit `Issue` trailer as enforced by {git_lint_link}. When no ID can be found, this will default to _All_ issues.
...
The manifest allows you to capture build details which can be useful when needing to compare differences between a previous and current build. For convenience, you can use the `Milestoner::Tags::Manifest` class to manage manifests. Example:
...
│   ├── _content.html.erb
│   ├── _content.xml.erb
│   ├── _contributors.adoc.erb
│   ├── _contributors.html.erb
│   ├── _contributors.md.erb
│   ├── _contributors.stream.erb
│   ├── _contributors.xml.erb
│   ├── _logo.adoc.erb
│   ├── _logo.html.erb
│   ├── _logo.md.erb
│   ├── _logo.xml.erb
